...
def _tls_verify_value():
    cert_file = str(os.getenv("SSL_CERT_FILE") or "").strip()
    req_bundle = str(os.getenv("REQUESTS_CA_BUNDLE") or "").strip()
    if cert_file:
        return cert_file
    if req_bundle:
        return req_bundle
    if str(os.getenv("TA_INSECURE_SSL", "")).strip().lower() in {"1", "true", "yes", "on"}:
        return False
    try:
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        llm_kwargs.setdefault("http_client", httpx.Client(verify=verify))
        llm_kwargs.setdefault("http_async_client", httpx.AsyncClient(verify=verify))

...
export function DialogueBoxPanel({ currentTicker, sceneRef }) {
    ...
    const sceneLiveLines = currentSceneKey
        ...
        : [];
    const pipelineSceneCount = Number(pipelineState?.scene_count ?? 0);
    const pipelineLatestSceneIndex = Number(pipelineState?.latest_scene_index ?? -1);
    const pipelineLlmCalls = Number(pipelineState?.llm_calls ?? 0);
    const pipelineToolCalls = Number(pipelineState?.tool_calls ?? 0);
    const sceneStreamAvailable = Boolean(
        pipelineState?.scene_stream_available ||
        (Number.isFinite(pipelineSceneCount) && pipelineSceneCount > 0) ||
        (Number.isFinite(pipelineLatestSceneIndex) && pipelineLatestSceneIndex >= 0)
    );
    const liveLineForAgent = sceneLiveLines.find((line) => (
        normalizeTradingAgentName(line?.agent) === currentAgentName
    ));
    const canonicalSceneStreamMissing = Boolean(
        isTradingAgentsMode
        && pipelineState?.active_run_id
        && String(pipelineState?.phase || '').toUpperCase() !== 'IDLE'
        && !activeScene
        && (!sceneLiveLines || sceneLiveLines.length === 0)
        && !sceneStreamAvailable
    );
    const upstreamFirstReportPending = Boolean(
        isTradingAgentsMode
        && pipelineState?.active_run_id
        && String(pipelineState?.phase || '').toUpperCase() !== 'IDLE'
        && !activeScene
        ...
        && sceneStreamAvailable
        && (
            pipelineLatestSceneIndex <= 0 ||
            pipelineSceneCount <= 1 ||
            pipelineLlmCalls > 0 ||
            pipelineToolCalls > 0
        )
    );
    ...
    if (activeScene && sceneDialogue.length > 0) {
        ...
        if (shouldShowTradingAgentsBootMessage) {
            ...
            rawMessage = canonicalSceneStreamMissing
                ? 'WARNING: Scene stream missing. Animation, pathing, dialogue paused.'
                : upstreamFirstReportPending
                    ? `Upstream analysis running for ${currentTicker || '---'}. First report pending.`
                : '';
            agentName = 'SYSTEM';
    ...
    useEffect(() => {
        ...
        const typingInterval = setInterval(() => {
            const nextChar = rawMessage.slice(0, i + 1);
            setDisplayedText(nextChar);

            if (rawMessage[i] && rawMessage[i] !== ' ') {
            }
    ...
    }, [rawMessage, currentDialogueIndex, isPaused, shouldPersistCanonicalTimelineScene, activeScene, sceneDialogue.length]);

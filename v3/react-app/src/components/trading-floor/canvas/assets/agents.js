...
function drawFaceDefault(paint, C, {
  hx,
  gy,
  hW,
  cx,
...
export function createDetailedAgent(scene, key, agentColor, isEvening = false) {
  ...
  const drawAgent = (ox, oy, row, cycle) => {
    ...
    if (isBack) {
      ...
      if ((row === 13 && cycle >= 2) || row === 14) {
        ...
        if (row === 13 && cycle === 1) { // Sad eyes for facepalm swing
          ...
          // Read frame uses custom glasses overlay later; avoid bright default face.
          drawDefaultFace = false
        }
        
        if (drawDefaultFace) {
          const isReadFrame = row === 5
          drawFaceDefault(paint, C, {
            hx,
            gy,
            hW,
            cx,
            glassW,
            glassH,
            lensTint: isReadFrame ? shadeColor(C.glassFrame, -26) : null,
            disableGlint: isReadFrame,
          })
        }
      }
    }
    ...
    if (row === 13) {
      ...
      // Reading mode glasses: keep lens tint, remove hard white flash.
      const gy = ty - 8
      const readLens = shadeColor(C.glassFrame, -26)
      r(readLens, cx - 8, gy + bnc, 7, 5)
      r(readLens, cx + 1, gy + bnc, 7, 5)
      r(C.glassFrame, cx - 8, gy + bnc, 7, 1)
      r(C.glassFrame, cx + 1, gy + bnc, 7, 1)
...
export function createBearAgent(scene, key, agentColor, isEvening = false) {
  ...
  const drawAgent = (ox, oy, row, cycle) => {
    ...
    if (row === 13) {
      ...
      const gy = ty - 10
      const readLens = shadeColor(C.glassFrame, -26)
      r(readLens, cx - 10, gy + bnc, 8, 6)
      r(readLens, cx + 2, gy + bnc, 8, 6)
      r(C.glassFrame, cx - 10, gy + bnc, 8, 1)
      r(C.glassFrame, cx + 2, gy + bnc, 8, 1)
    }
  }

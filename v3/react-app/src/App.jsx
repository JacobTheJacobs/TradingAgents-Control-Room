...
const isBackendShellPath = (pathname = '') => pathname === '/'
...
const normalizeTaAnimation = (value, fallback = 'idle') => {
  ...
  if (normalized === 'read') return 'sit_back'
  ...
  if (fallbackNormalized === 'read') return 'sit_back'
...
function AppContent() {
  ...
  const { 
    ...
    setLightMode, setPipelineState, addZoneEvent, setClosedTrades, setAnalytics,
    setSpyBenchmark, setZoneEvents, setActiveScene, setDialogue, addStreamedNews, appendLiveNews, setMarqueeText,
    refreshAgents, setTaRunStats, resetTaRunStats, setLiveTaReports
  } = useTradingFloor();
  ...
  const mergePipelineProgress = useCallback((prevState = {}, nextState = {}) => {
    ...
    const preserveIfEmpty = (key) => {
      ...
      if (hasPrev) merged[key] = prevValue
    }
    ;[
      'ticker',
      'current_ticker',
  ...
  const hydrateScenePayload = useCallback((scene = {}) => {
    ...
    const agentStations = { ...scene.agentStations }
    const agentAnimations = { ...scene.agentAnimations }
    const agentPaths = { ...scene.agentPaths }

    agents.forEach((agent) => {
      if (!agentStations[agent] && scene.location) {
        agentStations[agent] = scene.location
      }
      if (!agentAnimations[agent]) {
        agentAnimations[agent] = animations[agent] || animations.default || 'idle'
      }
      if (!agentPaths[agent]) {
        agentPaths[agent] = 'direct'
      }
    })
  ...
  const scenePackageToCommand = useCallback((scene = {}, trigger = 'tradingagents', meta = {}) => {
    ...
    return {
      ...
      state: scene.state,
      agents:
        (Array.isArray(scene.active_agents) && scene.active_agents.length > 0
          ? scene.active_agents
          : (scene.station_targets || []).map((item) => item?.agent).filter(Boolean)),
  ...
  const isLlmValidatedCanonicalScene = useCallback((sceneOrCommand = null) => {
    ...
    const hasSceneCoverage =
      (Array.isArray(sceneOrCommand?.lines) && sceneOrCommand.lines.length >= 12) ||
      (Array.isArray(sceneOrCommand?.dialogue) && sceneOrCommand.dialogue.length >= 12)
    if (writerMeta.validationPassed !== true) {
      const acceptedSources = new Set(['llm', 'auto_llm', 'deterministic'])
      if (writerMeta.writerSource && acceptedSources.has(writerMeta.writerSource) && hasSceneCoverage) {
        return true
      }
      const hasCanonicalMarkers = Boolean(
        sceneOrCommand?.sceneIndex != null ||
        sceneOrCommand?.scene_index != null ||
        sceneOrCommand?.sceneKind ||
        sceneOrCommand?.scene_kind ||
        sceneOrCommand?.scriptMeta?.scene_key ||
        sceneOrCommand?.script_meta?.scene_key
      )
      if (!writerMeta.writerSource && hasCanonicalMarkers && hasSceneCoverage) {
        return true
      }
      return false
    }
    const acceptedSources = new Set(['llm', 'auto_llm', 'deterministic'])
    if (!writerMeta.writerSource) return true
    return acceptedSources.has(writerMeta.writerSource)
  ...
  const shouldUseSynthTaScene = useCallback((payload = {}) => {
    void payload
    void hasCanonicalSceneForRun
    // TradingAgents now uses backend-authored canonical scenes only.
    return false
  ...
  const computeElapsedSeconds = (startTime, endTime, fallback = null) => {
    const startMs = Date.parse(startTime || '')
    if (!Number.isFinite(startMs)) return fallback
    const endMs = Date.parse(endTime || '')
    if (!Number.isFinite(endMs)) return fallback
  ...
  const syncTaRunStatsFromRunRecord = useCallback((run) => {
    ...
    const backendReportsCompleted = Number(run?.reports_completed ?? run?.raw_state?.reports_completed)
    const backendAgentsCompleted = Number(run?.agents_completed ?? run?.raw_state?.agents_completed)
    ...
    const sceneDerivedProgress = Math.max(
      0,
      Math.min(
    ...
    const reportSectionsProgress = Math.max(
      sceneDerivedProgress,
      Number.isFinite(backendReportsCompleted) ? Math.max(0, backendReportsCompleted) : 0,
      Number.isFinite(backendAgentsCompleted) ? Math.max(0, backendAgentsCompleted) : 0
    )
    ...
    setTaRunStats((prev) => {
      ...
      return {
        ...
        elapsed: isRunningRun
          ? base.elapsed
          : computeElapsedSeconds(
            runChanged ? startTime : (base.startTime || startTime),
            normalizeTimestamp(
              run?.completed_at ||
              run?.finished_at ||
              run?.ended_at ||
        ...
        reportsCompleted: reportSectionsProgress > 0
          ? Math.max(
            base.reportsCompleted || 0,
            Number.isFinite(backendReportsCompleted)
              ? Math.max(0, backendReportsCompleted)
              : Math.min(5, Math.ceil(reportSectionsProgress / 3))
          )
  ...
  const reconcileCompletedPackageLive = useCallback((pkg) => {
    ...
    if (runId && sceneHistory.length > 0) {
      ...
      if (latestScene) {
        ...
        const activeSceneRunId =
          state.activeScene?.runId ||
          state.activeScene?.run_id ||
          state.activeScene?.scriptMeta?.run_id ||
          null
        const activeSceneIndexRaw =
          state.activeScene?.sceneIndex ??
          state.activeScene?.scene_index ??
          state.activeScene?.scriptMeta?.scene_index
        const activeSceneIndex = Number(activeSceneIndexRaw)
        const latestSceneIndex = Number(latestSceneEntry?.scene_index)
        const shouldForceRehydrateLatestScene =
          !isCanonicalTradingAgentsSceneCommand(state.activeScene) ||
          activeSceneRunId !== runId ||
          (Number.isFinite(latestSceneIndex) && (!Number.isFinite(activeSceneIndex) || activeSceneIndex < latestSceneIndex))
        rememberCanonicalScene(
          {
            run_id: runId,
            active_run_id: runId,
        ...
        if (shouldForceRehydrateLatestScene) {
    ...
    if (isCompleted) {
      ...
      setTaRunStats((prev) => ({
        ...(function buildNext() {
          const resolvedStart =
            normalizeTimestamp(prev.startTime) ||
            normalizeTimestamp(pkg?.created_at || pkg?.start_time || pkg?.raw_state?.created_at || pkg?.raw_state?.start_time) ||
            prev.startTime ||
            null
          const resolvedEnd =
            normalizeTimestamp(
          ...
          return {
            ...
            startTime: resolvedStart,
            endTime: resolvedEnd,
            elapsed: computeElapsedSeconds(resolvedStart, resolvedEnd, prev.elapsed),
  ...
  const reconcileFromLatestRun = useCallback(async () => {
    ...
    try {
      ...
      const latest = payload?.run || payload
      ...
      const detailPayload = detailRes.ok ? await detailRes.json() : latest
      const detail = detailPayload?.run || detailPayload
      syncTaRunStatsFromRunRecord(detail)
      reconcileCompletedPackageLive(detail)
      const detailStatus = String(detail?.run_status || detail?.status || '').toUpperCase()
      const detailIsRunning = detailStatus === 'RUNNING'
      const sceneCount = Number(detail?.scene_count ?? detail?.raw_state?.scene_count ?? 0)
      const latestSceneIndex = Number(detail?.latest_scene_index ?? detail?.raw_state?.latest_scene_index ?? -1)
      const llmCalls = Number(detail?.llm_calls ?? detail?.raw_state?.llm_calls ?? 0)
      const toolCalls = Number(detail?.tool_calls ?? detail?.raw_state?.tool_calls ?? 0)
      const tokensIn = Number(detail?.tokens_in ?? detail?.raw_state?.tokens_in ?? 0)
      const tokensOut = Number(detail?.tokens_out ?? detail?.raw_state?.tokens_out ?? 0)
      setPipelineState((prev) => ({
        ...prev,
        active_run_id: detailIsRunning ? (detail?.run_id || prev?.active_run_id || null) : null,
        phase: detailIsRunning ? (prev?.phase || 'ANALYSTS') : (detailStatus || prev?.phase || 'IDLE'),
        status: detailIsRunning ? 'RUNNING' : (detailStatus || prev?.status || 'IDLE'),
        scene_count: Number.isFinite(sceneCount) ? Math.max(0, sceneCount) : (prev?.scene_count ?? 0),
        latest_scene_index: Number.isFinite(latestSceneIndex) ? latestSceneIndex : (prev?.latest_scene_index ?? -1),
        latest_scene_label: detail?.latest_scene_label || prev?.latest_scene_label || null,
        scene_stream_available: Boolean(
          (Array.isArray(detail?.scene_history) && detail.scene_history.length > 0) ||
          (Number.isFinite(sceneCount) && sceneCount > 0)
        ),
        llm_calls: Number.isFinite(llmCalls) ? Math.max(prev?.llm_calls || 0, llmCalls) : (prev?.llm_calls || 0),
        tool_calls: Number.isFinite(toolCalls) ? Math.max(prev?.tool_calls || 0, toolCalls) : (prev?.tool_calls || 0),
        tokens_in: Number.isFinite(tokensIn) ? Math.max(prev?.tokens_in || 0, tokensIn) : (prev?.tokens_in || 0),
        tokens_out: Number.isFinite(tokensOut) ? Math.max(prev?.tokens_out || 0, tokensOut) : (prev?.tokens_out || 0),
      }))
    ...
    clearCanonicalScenePlaybackQueue,
    playCanonicalSceneCommand,
    reconcileCompletedPackageLive,
    setPipelineState,
    syncTaRunStatsFromRunRecord,
  ])
  ...
  const clearIdleTradingAgentsLiveState = useCallback(() => {
    ...
    setPipelineState((prev) => ({
      ...
      max_attempts: 1,
      scene_count: 0,
      latest_scene_index: null,
      latest_scene_label: null,
      scene_stream_available: false,
      llm_calls: 0,
      tool_calls: 0,
      tokens_in: 0,
      tokens_out: 0,
        ...
        ws.onmessage = (event) => {
          ...
          switch (data.type) {
            ...
            case 'tradingagents_scene_generated': {
              ...
              const phaseKey = String(scene?.phase || '').trim()
              const sceneLines = Array.isArray(scene?.lines)
                ? scene.lines
                    .map((line) => ({
                      agent: normalizeTradingAgentName(line?.speaker || line?.agent) || line?.speaker || line?.agent,
                    ...
                    .filter((line) => line.agent && line.text)
                : []
              if (phaseKey && sceneLines.length > 0) {
                liveStepDialogueRef.current = {
                  ...liveStepDialogueRef.current,
                  ...normalizeLiveDialogueMap({
                    [phaseKey]: sceneLines,
            ...
            case 'run_failed':
              if (data.type === 'pipeline_start') {
                const pipelineRunId = data?.active_run_id || data?.run_id || null
                const hasAcceptedCanonicalScene = hasCanonicalSceneForRun(pipelineRunId)
                if (!hasAcceptedCanonicalScene) {
                  resetCanonicalSceneHistory(pipelineRunId)
                  clearCanonicalScenePlaybackQueue()
                }
                if (data.agent_behavior_defaults) applyAgentBehaviorDefaults(data.agent_behavior_defaults)
                ...
                resetTaRunStats({
                  ...
                  invalidAgents: [],
                  upstreamGeneratedAt: null,
                  startTime: normalizeTimestamp(data.start_time) || new Date().toISOString(),
                  endTime: null,
                  elapsed: null,
                });
                setPipelineState((prev) => ({
                  ...prev,
                  active_run_id: pipelineRunId || prev?.active_run_id || null,
                  scene_count: hasAcceptedCanonicalScene ? (prev?.scene_count ?? 0) : 0,
                  latest_scene_index: hasAcceptedCanonicalScene ? (prev?.latest_scene_index ?? null) : null,
                  latest_scene_label: hasAcceptedCanonicalScene ? (prev?.latest_scene_label ?? null) : null,
                  scene_stream_available: hasAcceptedCanonicalScene ? Boolean(prev?.scene_stream_available) : false,
                  llm_calls: hasAcceptedCanonicalScene ? (prev?.llm_calls || 0) : 0,
                  tool_calls: hasAcceptedCanonicalScene ? (prev?.tool_calls || 0) : 0,
                  tokens_in: hasAcceptedCanonicalScene ? (prev?.tokens_in || 0) : 0,
                  tokens_out: hasAcceptedCanonicalScene ? (prev?.tokens_out || 0) : 0,
                }));
              } else {
                addMessage(data);
              }
              ...
              if (data.type === 'agent_action' && data.scene_stage === 'start' && shouldUseSynthTaScene(data)) {
                triggerTradingAgentsScene(data);
                ...
                const completedLlmCalls = Number(data?.llm_calls)
                const completedToolCalls = Number(data?.tool_calls)
                const completedTokensIn = Number(data?.tokens_in)
                const completedTokensOut = Number(data?.tokens_out)
                setTaRunStats(prev => ({
                  ...(function buildNext() {
                    const resolvedEnd =
                      normalizeTimestamp(
                      ...
                      prev.endTime ||
                      null
                    const resolvedStart = normalizeTimestamp(prev.startTime) || prev.startTime || null
                    const resolvedElapsed = computeElapsedSeconds(resolvedStart, resolvedEnd, prev.elapsed)
                    return {
                      ...prev,
                      running: false,
                      completed: true,
                      ...
                      llmCalls: Number.isFinite(completedLlmCalls) ? Math.max(0, completedLlmCalls) : (prev.llmCalls || 0),
                      toolCalls: Number.isFinite(completedToolCalls) ? Math.max(0, completedToolCalls) : (prev.toolCalls || 0),
                      tokensUp: Number.isFinite(completedTokensIn) ? Math.max(0, completedTokensIn) : (prev.tokensUp || 0),
                      tokensDown: Number.isFinite(completedTokensOut) ? Math.max(0, completedTokensOut) : (prev.tokensDown || 0),
                      ...
                      reportsCompleted: Math.max(prev.reportsCompleted || 0, prev.reportsTotal || 12),
                    }
                  })(),
                }));
                ...
                setTaRunStats(prev => ({
                  ...(function buildNext() {
                    const resolvedEnd = normalizeTimestamp(data?.timestamp) || prev.endTime || null
                    const resolvedStart = normalizeTimestamp(prev.startTime) || prev.startTime || null
                    const resolvedElapsed = computeElapsedSeconds(resolvedStart, resolvedEnd, prev.elapsed)
                    return {
                      ...prev,
  ...
  return (
    <>
      <Routes>
        ...
        <Route path="/admin/god" element={<Navigate to="/" replace />} />

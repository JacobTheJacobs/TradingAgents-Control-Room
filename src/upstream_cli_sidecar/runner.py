...
@contextmanager
def _deadline_timeout(seconds: int):
    if seconds <= 0:
        yield
        return
    ...
    def _handler(_signum, _frame):
        timed_out["hit"] = True
        raise TimeoutError(f"Upstream execution timed out after {seconds}s")
...
def _is_transient_error(error_code: str, message: str, tb: str) -> bool:
    ...
    if error_code.startswith("HTTP_5"):
        return True
    if error_code in {"UPSTREAM_TIMEOUT", "TRANSPORT_ERROR", "HTTP_504"}:
        return True
    return any(
        token in merged
        for token in (
...
def main() -> int:
    ...
    _emit({"type": "pipeline_start", "run_id": run_id, "timestamp": _now_iso()})
    ...
    try:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                with _deadline_timeout(timeout_seconds):
                ...
                try:
                    ta._log_state(args.date, final_state)
                except Exception:
                    pass

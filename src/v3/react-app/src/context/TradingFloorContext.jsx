...
export function TradingFloorProvider({ children }) {
  ...
  const broadcastUISettings = useCallback(async (overrides = {}) => {
    try {
      const payload = {
        hideNews: overrides.hideNews !== undefined ? overrides.hideNews : state.hideNews,
        hideCycle: overrides.hideCycle !== undefined ? overrides.hideCycle : state.hideCycle,
        hideLeftSidebar: overrides.hideLeftSidebar !== undefined ? overrides.hideLeftSidebar : state.hideLeftSidebar,
        hideRightSidebar: overrides.hideRightSidebar !== undefined ? overrides.hideRightSidebar : state.hideRightSidebar,
        showPerformanceView: overrides.showPerformanceView !== undefined ? overrides.showPerformanceView : state.showPerformanceView,
        lightMode: overrides.lightMode !== undefined ? overrides.lightMode : state.lightMode,
        marqueeSpeed: overrides.marqueeSpeed !== undefined ? overrides.marqueeSpeed : state.marqueeSpeed,
        newsScrollSpeed: overrides.newsScrollSpeed !== undefined ? overrides.newsScrollSpeed : state.newsScrollSpeed
      }

      await fetch('/api/admin/broadcast-ui-settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      })
    } catch (e) {
      console.error('Failed to broadcast UI settings:', e)
    }
  }, [state.hideNews, state.hideCycle, state.hideLeftSidebar, state.hideRightSidebar, state.showPerformanceView, state.lightMode, state.marqueeSpeed, state.newsScrollSpeed])

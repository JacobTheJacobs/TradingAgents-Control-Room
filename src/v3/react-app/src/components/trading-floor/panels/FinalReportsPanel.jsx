...
import { startClientRunner } from '../../../services/runnerClient'
import { getRunnerKeyForProvider } from '../../../utils/runnerKeyStore'
import {
  TRADING_AGENT_BY_ID,
...
export default function FinalReportsPanel({ tabLabel = '' }) {
  ...
  const loadLatestCompletedRun = useCallback(async (signal) => {
    ...
    try {
      ...
      if (!isCompletedTradingAgentsRun(latestRun)) {
        const listResponse = await fetch(`${RUNS_API_BASE}?limit=20`, {
          cache: 'no-store',
          signal,
        })
        ...
        if (listResponse.ok) {
          ...
          latestRun = (listPayload?.runs || []).find((run) => isCompletedTradingAgentsRun(run)) || null
        } else {
          latestRun = null
        }
      }
  ...
  useEffect(() => {
    ...
    const handleDecisionUpdate = (event) => {
      ...
      setSessionCompletedRun(summary)
  ...
  const liveDecisionForRun =
    activeRunId && liveDecisionPackage?.run_id === activeRunId
      ? liveDecisionPackage
      : null
  const liveTicker = String(pipelineState.ticker || pipelineState.current_ticker || liveDecisionForRun?.ticker || '').toUpperCase()
  ...
  const liveReportCards = useMemo(() => {
    ...
    return reportSections.map((section, index) => {
      ...
      const isComplete = Boolean(hasContent || hasTelemetryCompletion)

      let status = 'pending'
      if (isLivePackageCompleted) {
  ...
  const handleRetryRun = useCallback(async () => {
    ...
    const provider = String(pipelineState?.llm_provider || 'nvidia').trim().toLowerCase()
    const apiKey = getRunnerKeyForProvider(provider)
    if (provider !== 'ollama' && !apiKey) {
      setRetryError(`Missing API key for provider: ${provider}`)
      return
    }
    setRetryError('')
    ...
    try {
      const runParams = {
        ...
        dramaLevel: String(pipelineState?.drama_level || 'medium').toLowerCase(),
        apiKey,
      }
        ...
        onProgress: (event) => {
          ...
          if (event?.type === 'start') {
            ...
            setPipelineState((prev) => ({
              ...
              trade_date: runParams.date,
              llm_provider: runParams.provider,
              quick_model: runParams.quickModel,
              deep_model: runParams.deepModel,
              output_language: runParams.outputLanguage,
              drama_level: runParams.dramaLevel,
              research_depth: runParams.depth,
              active_run_id: event.runId || prev?.active_run_id || null,
              current_step: 'market_analyst',
              agent_display_name: 'Market Analyst',
              action: `Retry started for ${ticker}`,
              status: 'STARTING',
              timestamp: now,
            }))
            ...
            const action = String(event?.package?.model_action || event?.package?.recommended_action || 'HOLD').toUpperCase()
            setTaRunStats((prev) => ({
              ...prev,
            ...
            setPipelineState((prev) => ({
              ...
              current_phase: 0,
              status: event.type === 'aborted' ? 'ABORTED' : 'FAILED',
              action: event.type === 'aborted' ? 'Run aborted by user.' : (event.error || 'Retry failed.'),
              timestamp: now,
            }))
          }
        },
  ...
  const recommendationAction = ['BUY', 'SELL', 'HOLD'].includes(normalizedRecommendation)
  ...
  const hasFinalDecisionSignal = Boolean(
    liveDecisionForRun?.raw_state?.final_trade_decision ||
    liveDecisionForRun?.raw_state?.final_decision ||
    liveDecisionForRun?.final_decision ||
    liveDecisionForRun?.recommended_action ||
  ...
  const liveHeroAction = hasFinalDecisionSignal
    ? liveActionKeyword
    : ''
  const liveHeroTicker = (liveTicker || pipelineState?.ticker || pipelineState?.current_ticker || '----').toUpperCase()
  ...
  const renderReportCommandDeck = ({
    ...
    allowTerminalStatus = mode !== 'live',
  } = {}) => (
    (() => {
      const normalizedDecision = String(decisionLabel || '').trim().toUpperCase()
      const hasTerminalAction = normalizedDecision === 'BUY' || normalizedDecision === 'SELL' || normalizedDecision === 'HOLD'
      const resolvedStatusLabel =
        mode === 'live'
          ? ((allowTerminalStatus && hasTerminalAction)
            ? normalizedDecision
            : ((allowTerminalStatus && normalizedDecision === 'COMPLETED') ? 'COMPLETED' : 'IN PROGRESS'))
          : mode === 'idle'
            ? 'IDLE'
        ...
        <div className="final-reports-panel__live">
          <div className="final-reports-panel__live-body final-reports-panel__live-body--grid">
            {renderReportCommandDeck({
              ...
              decisionLabel: hasFinalDecisionSignal && liveHeroAction ? liveHeroAction : '',
              decisionTone: getActionTone((hasFinalDecisionSignal && liveHeroAction) ? liveHeroAction : ''),
              allowTerminalStatus: hasFinalDecisionSignal || isLivePackageCompleted,
        ...
        <div className="final-reports-panel__live">
          <div className="final-reports-panel__live-body final-reports-panel__live-body--grid">
            {renderReportCommandDeck({
              ...
              decisionLabel: hasFinalDecisionSignal && liveHeroAction
                ? liveHeroAction
                : (isLivePackageCompleted ? (recommendationAction || 'COMPLETED') : ''),
              decisionTone: getActionTone(
                hasFinalDecisionSignal && liveHeroAction
                  ? liveHeroAction
                  : (isLivePackageCompleted ? (recommendationAction || 'COMPLETED') : '')
              ),
              allowTerminalStatus: hasFinalDecisionSignal || isLivePackageCompleted,

    ...
    switch (cmd.type) {
      ...
      case SceneCommandType.PLAY_STEP_SCENE:
        this.playStepScene(
          ...
          cmd.agents,
          cmd.location,
          cmd.animations || cmd.agentAnimations,
          cmd.agentStations || cmd.stations,
          cmd.agentAnimations || cmd.animations,
          cmd.agentPaths || cmd.paths,
          cmd.highlight,
          cmd.highlightAgents,
          cmd.directTargets || cmd.direct_targets
        )
        break
...
export const broadcastSceneCommand = async (cmd) => {
  ...
  try {
    ...
    console.error('[Showrunner] Failed to broadcast scene command:', error)
    return false
  }
}

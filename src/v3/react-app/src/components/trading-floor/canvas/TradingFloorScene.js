...
import { startCatBehavior } from './catBehavior'
...
export class TradingFloorScene extends Phaser.Scene {
  ...
  async create() {
    ...
    // Sync map from API on start
    await this.syncRoomMap({ rerender: false });
    ...
    try {
      ...
      console.log("Phaser Scene: Creating Cats...");
      this.createCats()
  ...
  deployAgentsToStations() {
    ...
    Object.keys(this.agents).forEach(name => {
      ...
      if (this.movementManager) {
        this.movementManager.moveAgent(name, stationType, MovePriority.AUTOMATED, 'initial_spawn_deployment');
      }
    });
  }
    ...
    for (let row = 0; row < roomMap.length; row++) {
      for (let col = 0; col < roomMap[0].length; col++) {
        ...
        if (type === TILE_TYPES.WALL) {
          ...
          addRoomSprite(this.add.image(x, y + 4, 'money').setDepth(y))
  ...
  createAgents() {
    ...
    agentList.forEach(([name, config], index) => {
      const fallbackSpots = this.getGatherSpots('table') || GATHER_SPOTS.table
      const fallbackGridPoint = fallbackSpots[index % fallbackSpots.length]
      const spawnX = Number(config?.position?.x) || ((fallbackGridPoint.c * 32) + 16)
      const spawnY = Number(config?.position?.y) || ((fallbackGridPoint.r * 32) + 16)
      ...
      agent.homePos = getGridPos(spawnX, spawnY)
      agent.fatigue = 0
      agent.status = 'idle'
      agent.active = true
  ...
  createCats() {
    ...
    if (this.cats) {
      this.cats.forEach(cat => {
        if (cat && cat.destroy) cat.destroy()
      })
    }
    this.cats = []
    ...
    for (let row = 0; row < roomMap.length; row++) {
      for (let col = 0; col < roomMap[0].length; col++) {
        if (roomMap[row][col] === TILE_TYPES.CAT) {
          catSpawns.push({ x: col * TILE_SIZE + TILE_SIZE / 2, y: row * TILE_SIZE + TILE_SIZE / 2 })
        }
      }
    }

    catSpawns.forEach(spawn => {
      const cat = this.add.sprite(spawn.x, spawn.y, 'cat')
      cat.setDepth(cat.y)
      cat.play('cat_idle')
      cat.setTint(Phaser.Math.Between(0x888888, 0xFFFFFF))
      cat.active = true
      this.cats.push(cat)
      startCatBehavior(this, cat)
    })
  }
  ...
  update(time, delta) {
    ...
    try {
      // Y-sorting for dynamic objects only (agents and cats)
    ...
    try {
      ...
      logUpdateError(error, 'cat-depth')
    }
  ...
  */
  ...
  /**
   * Setup annotation mode input handlers
   * Called when blueprint mode is enabled
   */
  setupBlueprintMode() {
    // Ghost Layer: Never enable annotation tools on OBS stream
    if (this.mode === 'obs') {
      console.log('[TradingFloorScene] Ghost Layer: Annotation tools disabled in OBS mode')
      return
    }
  ...
  handleAnnotationPointerDown(pointer) {
    // Ghost Layer: Never process annotation input on OBS stream
    if (this.mode === 'obs') return
  ...
  async syncRoomMap({ rerender = true } = {}) {
    try {
      const res = await fetch('/api/admin/map')
      if (res.ok) {
        const mapData = await res.json()
        if (mapData && Array.isArray(mapData) && mapData.length > 0 && Array.isArray(mapData[0])) {
          console.log('[Phaser] Map synced from API')
          this.setRoomMap(mapData)
          this.refreshGatherSpots()
          if (rerender) this.rerenderRoom()
        }
      }
    } catch (e) {
      console.warn('[Phaser] Failed to sync map from API:', e)
    }
  }
